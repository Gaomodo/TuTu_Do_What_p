...
#include <system_bl808.h>   // Interrupt_Handler_Register
#include <bl_mtd.h> 
#include <bl808_pds.h>
...
#define PIN_BTN_TEST  22
static TaskHandle_t xGpioTask = NULL;
static volatile int btn_count = 0;
static volatile bool isc906_sleeping = true;
extern TaskHandle_t capture_worker;
volatile int g_wifi_got_ip = 0;
volatile int g_c906_sleeping = 0; 
volatile int g_c906_booting = 0; 
...
static void my_gpio_int_Init()
{
          /* ====== GPIO 中断 ====== */
      GLB_GPIO_Cfg_Type cfg = {0};
      cfg.gpioPin  = PIN_BTN_TEST;
...
static void gpio_btn_isr(void)
{
    if (SET == GLB_Get_GPIO_IntStatus(PIN_BTN_TEST)) {
        GLB_Clr_GPIO_IntStatus(PIN_BTN_TEST);
        btn_count++;
        BaseType_t wake = pdFALSE;
        vTaskNotifyGiveFromISR(xGpioTask, &wake);
        portYIELD_FROM_ISR(wake);
    }
}
static void c906_power_on_and_resume(void)
  {
     bl_mtd_handle_t handle_d0fw;
     bl_mtd_info_t info;
     uint32_t start_addr = 0x50100000;
      
    PDS_Power_On_MM_System();        // SDK 自带: 恢复 MM 系统电源

      if (bl_mtd_open("D0FW",&handle_d0fw,BL_MTD_OPEN_FLAG_BUSADDR))
        {
            printf("open D0FW failed\r\n");
            return;
        }

    bl_mtd_info(handle_d0fw,&info);
    memcpy((void *)start_addr,info.xip_addr,info.size);
    ...
    hal_boot_cpu0(start_addr);
      ...
      printf("[PWR] C906 resumed\r\n");
  }



    static void c906_sleep(void)
  {
    g_c906_sleeping = 1;
    if (capture_worker) {
          vTaskSuspend(capture_worker);   // ★ 立即挂起 mqtt, 它不再执行任何指令
      }

//     /* ★ 关 sensor 电源 (LDO15CIS), 让冷启动时 sensor 真正 POR */
//       uint32_t ldo_val = BL_RD_REG(GLB_BASE, GLB_LDO15CIS);
//       ldo_val = BL_CLR_REG_BIT(ldo_val, GLB_PU_LDO15CIS);   // bit0=0 断电
//       BL_WR_REG(GLB_BASE, GLB_LDO15CIS, ldo_val);

//   // 关 MIPI PHY LDO11 电源（dphy_config_15 @ 0x3001a1bc 的 bit 8）
//   uint32_t dphy_val = BL_RD_WORD(0x3001a1bc);   // 直接读地址
//   dphy_val &= ~(1U << 8);                        // 清 bit 8 (reg_dphy_pu_ldo11 = 0)
//   BL_WR_WORD(0x3001a1bc, dphy_val);              // 直接写地址

      GLB_Halt_CPU(GLB_CORE_ID_D0);


      PDS_CTL2_Type ctl2 = {0};
      ctl2.forceDspGateClk = 1;    // 关MM时钟
      ctl2.forceDspMemStby = 1;    // PSRAM 进低功耗 standby
      PDS_CTL3_Type ctl3 = {0};
      PDS_CTL5_Type ctl5 = {0};
      PDS_Force_Config(&ctl2, &ctl3, &ctl5);

      printf("[PWR] C906 sleep (halt + gate_clk + mem_stby)\r\n");
  }

  static void c906_wakeup(void)
  {
    g_c906_sleeping = 0;
...
//       /* ★ 先恢复 sensor 电源 (LDO15CIS) */
//       uint32_t ldo_val = BL_RD_REG(GLB_BASE, GLB_LDO15CIS);
//       ldo_val = BL_SET_REG_BIT(ldo_val, GLB_PU_LDO15CIS);   // bit0=1 上电
//       BL_WR_REG(GLB_BASE, GLB_LDO15CIS, ldo_val);

//       /* 恢复 MIPI PHY LDO11 电源 */
//   uint32_t dphy_val = BL_RD_WORD(0x3001a1bc);
//   dphy_val |= (1U << 8);                         // 设 bit 8 (reg_dphy_pu_ldo11 = 1)
//   BL_WR_WORD(0x3001a1bc, dphy_val);


      arch_delay_us(500);
      c906_power_on_and_resume();
          if (capture_worker) {
          vTaskResume(capture_worker);   // ★ C906 唤醒后恢复 mqtt
      }
  }
static void vGpioTask(void *pv)
{
    while (1) {
        ...
        int level = GLB_GPIO_Read(PIN_BTN_TEST);
          if (isc906_sleeping ) {
              // C906 关着 → 开机
              c906_wakeup();
              printf("[PWR] C906 booted\r\n");
              isc906_sleeping = false;
          } else if(!isc906_sleeping) {
              // C906 开着 → 关机
              c906_sleep();
              printf("[PWR] C906 sleeping\r\n");
              isc906_sleeping = true;
          }
              GLB_Clr_GPIO_IntStatus(PIN_BTN_TEST);
...
static void cmd_wifi_conn(char *buf, int len, int argc, char **argv)
{
    wifi_sta_connect(argv[1], argv[2]);  // 直接连!
}
...
const static struct cli_command cmds_user[] STATIC_CLI_CMD_ATTRIBUTE = {
    ...
    {"wifi", "wifi", cmd_wifi},
    {"wifi_conn", "WiFi Connect", cmd_wifi_conn},
};
...
void bfl_main()
{
    ...
    xTaskCreateStatic(aos_loop_proc, TASK_AOS_TASKNAME, TASK_AOS_STACKSIZE, NULL, TASK_AOS_PRIORITY,
                      aos_loop_proc_stack, &aos_loop_proc_task);
      Interrupt_Handler_Register(GPIO_INT0_IRQn, gpio_btn_isr);

      xTaskCreate(vGpioTask, "gpio", 512, NULL, 2, &xGpioTask);
